...
void MyTempMonAdpater::NotifyTempChg(float temp)
{
  ...
  // TR_PRINT_STR(m_trPort, trLevel, thingSpeakStatusString(status)); // m_trPort is non-static, cannot use in static method

  // m_tm1638 is non-static, cannot use in static method
  // If you want to display on TM1638, pass the pointer as an argument
  // Example usage: MyTempMonAdpater::NotifyTempChg(temp, tm1638);
}
